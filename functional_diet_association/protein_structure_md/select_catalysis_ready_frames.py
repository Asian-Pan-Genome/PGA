#!/usr/bin/env python3
"""Measure L1–L5 catalysis-ready water geometry in PGA–peptide trajectories."""
...
def discover_inputs(data_dir: Path) -> pd.DataFrame:
    rows = []
    for system in EXPECTED_MARKERS:
        for number in range(1, 6):
            replica = f"rep{number}"
            directory = data_dir / system / replica
            topology = directory / "step5_ref.tpr"
            trajectory = directory / "md_fit_20_100ns.xtc"
...
def main() -> None:
    args = parse_args()
    records = discover_inputs(args.data_dir.resolve())
